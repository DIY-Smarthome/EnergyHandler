...
const eh = new Eventhandler(8000, "dashboard");
eh.init().then(()=>subscribe());
...
async function sendOutletChange(device_id, outlet_id, change) {
    let response = await (await eh.request("set_outlet_status", {device: device_id, outlet: outlet_id, status_change: change})).getContentsFlat()[0];
    if (response) {
        return response;
    } else {
        return false;
...
async function checkUsage(device_id) {
    ...
    let tmp = response.getContentsFlat()[0];
    let current_settings = readDeviceFile(device_id);
    console.log(`Checking ${tmp}/${current_settings['usage']}`)
